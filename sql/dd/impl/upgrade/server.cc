...
bool setup_percona_telemetry(THD *thd [[maybe_unused]]) {
  ...
  const char **query_ptr = opt_percona_telemetry_disable ? &percona_telemetry_uninstall[0] : &percona_telemetry_install[0];
  for (; *query_ptr != nullptr; query_ptr++)
    if (ignore_error_and_execute(thd, *query_ptr)) {
      LogErr(WARNING_LEVEL, ER_LOG_PRINTF_MSG, "Failed during setup Percona Telemetry component. Ignoring.");
      break;
    };
  ...
  if (dd::end_transaction(thd, err)) {
      LogErr(WARNING_LEVEL, ER_LOG_PRINTF_MSG, "Failed to setup Percona Telemetry component. Ignoring.");
  }

  // Always return success, regardless of Percona Telemetry Component setup status.
  return false;
}
