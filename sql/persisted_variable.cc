...
Item* Persisted_variables_cache::create_item(THD *thd, const string &str) {
  ...
  if(!error) {
    return new (thd->mem_root)
        Item_int(str.c_str(), (uint)str.length());
  }
  return new (thd->mem_root) Item_string(
        str.c_str(), str.length(), &my_charset_utf8mb4_bin);
}
  ...
  for (auto iter = sorted_vars.begin(); iter != sorted_vars.end(); iter++) {
    ...
    switch (sysvar->show_type()) {
      ...
      case SHOW_CHAR:
      {
        ...
        Sys_var_multi_enum *multi_enum_var = dynamic_cast<Sys_var_multi_enum*>(sysvar);
